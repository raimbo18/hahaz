...
class LineTimeline(object):
    ...
    def __init__(self):
        if self.isLogin == True and self.channelId == self.server.CHANNEL_ID['LINE_TIMELINE']:
            self.client.log('[%s] : LineTimeline attached' % self.client.profile.mid)
    ...
    @loggedIn
    def createPost(self, text,mid,s):
        params = {'homeId': mid, 'sourceType': s}
        url = self.server.urlEncode(self.server.LINE_TIMELINE_API, '/v33/post/create.json', params)
        payload = {'postInfo': {'readPermission': {'type': 'ALL'}}, 'sourceType': s, 'contents': {'text': text}}
    ...
    def addImageToAlbum(self, mid, albumId, path):
        ...
        url = self.server.urlEncode(self.server.LINE_OBS_DOMAIN,'/album/a/upload.nhn',file)
