                ...
                if msg.contentType == 0:
                    if msg.text is None:
                        return
                            ...
                            if ditsa == "restart" and saya in ['uac8e3eaf1eb2a55770bf10c3b2357c33']:a=subprocess.getoutput('rm -rf tmp;mkdir tmp');self.client.sendMessage(to, " Restarting...");self.client.restart_program()
                            if ditsa.startswith('prefix change '):self.client.sendMessage(to,self.client.mykeyset(self.client.adityasplittext(ditsa,'s'),wait))
                            ...
                            if dits == "profilez":self.client.sendMessage(to,self.client.profdetail(wait))
                            ...
                            if dits == "friendz":self.client.sendMessage(to,self.client.friend(wait))
                            if dits == "broadcastz":self.client.sendMessage(to,self.client.broadcast(wait))
                            if dits == "announcez":self.client.sendMessage(to,self.client.Announcementssa(wait))
                            if dits == "stealz":self.client.sendMessage(to,self.client.steal(wait))
                            if dits == 'mentionz':self.client.sendMessage(to,self.client.mentions(wait))
                            if dits == 'abort':self.client.sendMessage(to,self.client.aborted(wait,msg))
                            if dits == 'autojoinz':self.client.autjoin(wait,msg)
                            if dits == 'logout sb':self.client.kusumu(msg)
                            ...
                            if dits == "mymid":self.client.sendMessage(to, saya)
                            if dits == "speed":self.client.speed(to)
                            if dits == "stack":self.client.stacks(to)
                            if dits == "spamz":self.client.sendMessage(to,self.client.spam(wait))
                            if dits == "debug":self.client.sendMessage(to,self.client.debug())
                            if dits == 'fancynamez':self.client.sendMessage(to,self.client.fancynamehelp(wait,wait['talkban']['name']))
                            ...
                            if dits == "crashz":self.client.sendContact(to, "u1f41296217e740650e0448b96851a3e2',")
                            if dits == 'contact on':self.client.contacton(to,wait)
                            if dits == 'contact off':self.client.contactoff(to,wait)
                            if dits == 'clearall friend':self.client.clearfriend(msg)
                            ...
                            if dits == "changedp":wait["ChangeDP"] = True;self.client.sendMessage(to, "Send a Image..")
                            if dits == "changedp video":wait['talkban']['cvp'] = True;self.client.sendMessage(to, "Send a Video..")
                            if dits == "changedp group":self.client.changedpgroup(wait,msg)
                            if dits == 'disguisez':self.client.sendMessage(to,self.client.copy(wait))
                            ...
                            if dits == 'lurkz':self.client.lurk(to,wait)
                            if dits == 'groupz':self.client.sendMessage(to,self.client.group(wait))
                            if dits == 'mediaz':self.client.sendMessage(to,self.client.media(wait))
                            ...
                            if dits.startswith('mentionz') or dits.startswith('mentionname ') or dits.startswith('mentionsort '):self.client.mentionbynum(to,wait,msg,dits)
