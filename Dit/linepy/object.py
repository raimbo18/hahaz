...
import json, time, ntpath,tempfile
...
class Object(object):
    ...
    def updateProfileVideoPicture(self, path):
        try:
            from ffmpy import FFmpeg;import traceback
            files = {'file': open(path, 'rb')}
    ...
    def uploadObjSquare(self, squareChatMid, path, type='image', returnAs='bool'):
        ...
        r = self.server.postContent(self.server.LINE_OBS_DOMAIN + '/r/g2/m/upload.nhn', data=data, headers=headers)
        if r.status_code != 201:
